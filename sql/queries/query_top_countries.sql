-- Top 3 Countries by Market (Sales vs Profit)
-- Are the top countries by Sales the same as the top countries by profit?
...
FROM country_figures;


-- Export Top 3 Countries by Sales by Market
COPY (
    SELECT
        market,
        sales_rank,
        country,
        sales
    FROM v_country_rankings
    WHERE sales_rank <= 3
    ORDER BY 1, 2 ASC
)
TO 'C:\temp\global_store\queries\top_countries_by_sales.csv'
WITH (
    FORMAT CSV,
    HEADER TRUE,
    ENCODING 'WIN1252'
...
-- Export Top 3 Countries by Profit by Market
COPY (
    SELECT
        market,
        profit_rank,
...
TO 'C:\temp\global_store\queries\top_countries_by_profit.csv'
WITH (
    FORMAT CSV,
    HEADER TRUE,
    ENCODING 'WIN1252'
